...
这里我现在有点异或，但是官网讲得很清楚：redis 采用的是**异步复制**，怎么理解**同步**和**异步复制**之间的关系呢？
...
**注意**：redis-sentinel **只是用来初始化一个 sentinel**，说白了就是用来添加 sentinel 的，第 2 点并不能添加master，因为 master 本来就有了，这里只是跟 sentinel 说你要监视谁而已。
...
6379 是源，待会下面会让你**输入要重新分片的槽的个数**，以及**目标节点的 id**。
...
跟 sentinel 的选举领头羊的模式差不多，最后票数超过一半的节点成为主节点。
