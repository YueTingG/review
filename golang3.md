...
什么意思呢？假设我现在申请了一个 `[]int64`，新申请的大小是 5，一个 int64 的大小是 8 字节，那么总共它需要 40 字节，但是你如果你看了 go 的内存管理，你会发现 go 只允许申请它内置定好的内存大小，例如 48 就是它定好的大小，就是下面的 size_to_class，那么只能向上取整。

最终，内存对齐之后，它申请的大小是 48 字节，那么纵隔申请了多少个数组呢？总大小/变量类型大小=数组个数，可得 48/8=6，所以最终申请了 6 个数组，而不是一开始的 5。
...
你想想嘛，我现在要申请一块内存，是不是为了一个对象去申请一块内存也就是 new 之类的，最终是不是落到了某一个 mspan 上面？具体现在是那一块 mspan，是不是根据你要申请的 size 来决定？然后最后确定是要哪块 mspan!
